...
const SavedItemsPanel = () => {
  const { user, supabaseReady, loading: authLoading } = useAuth()
  ...
  const handleSaveCalculation = async () => {
    ...
    try {
      const { error } = await saveCalculation({
        ...
        payload: snapshot,
        results: totals,
        isPublic: shareCalculationPublic,
      })
      if (error) throw error
    ...
    <section className="rounded-2xl border border-slate-200 bg-white/95 p-6 shadow-sm">
      ...
      <div className="mt-6">
        {activeTab === "qualities" ? (
                  ...
                  <div className="min-w-[220px] flex-1">
                    ...
                    <div className="mt-1 flex items-center gap-2">
                      {item.isPublic ? (
                        ...
                        <Badge tone="shared">Shared</Badge>
                      )}
                    </div>
                  </div>
          ...
          <div className="space-y-3">
            {calculationList.length === 0 && !loading ? (
              <p className="rounded-lg border border-dashed border-slate-300 bg-slate-50 p-6 text-sm text-slate-500">
